...
class SherlockValidString
{
  ...
  {
    Map<Long, Long> charCountCounts = s.chars()
        .mapToObj(c -> (char) c)
    ...
    // 1: Difference between frequencies is ONE and larger frequency occurred ONCE (delete once from larger freq)
    // e.g. aa bb ccc -> delete 1 c
    // 2: Smaller freq is ONE and occurred ONCE (remove the smaller freq)
    // e.g. aa bb c -> remove the c
    List<Map.Entry<Long, Long>> list = charCountCounts.entrySet().stream().sorted(comparingLong(Map.Entry::getKey)).collect(toList());
    ...
    return (largerCount - smallerCount == 1 && largerCountCount == 1) || (smallerCount == 1 && smallerCountCount == 1);
  }
