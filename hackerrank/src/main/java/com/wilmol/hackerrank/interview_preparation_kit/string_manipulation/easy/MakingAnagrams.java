 ...
 * <p>
 * Count number of character deletions required to make the strings anagrams of each other.
 * <p>
 * Runtime: O(n) (~3 passes)
 */
class MakingAnagrams
{
  ...
  static int makeAnagram(String a, String b)
  {
    ...
    aCharCounts.forEach((key, value) -> charCountDifference(bCharCounts, absCharDifferences).accept(key, value));

    bCharCounts.entrySet().stream()
        // already processed those in String a
        .filter(not(e -> aCharCounts.containsKey(e.getKey())))
        .forEach(e -> charCountDifference(aCharCounts, absCharDifferences).accept(e.getKey(), e.getValue()));
  ...
  private static BiConsumer<Character, Long> charCountDifference(Map<Character, Long> thatMap, Map<Character, Long> aggregator)
  {
